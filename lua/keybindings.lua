...
map("n", "<F5>", ":!clang++ -O2 -std=c++17 -fsanitize=address -I ~/solution/header -DLOCAL -DDBG_MACRO_NO_WARNING %:p -o %:p:r && %:p:r<CR>", opt)
map("n", "<F6>", ":!clang++ -O2 -std=c++17 -fsanitize=address -I ~/solution/header -DLOCAL -DDBG_MACRO_NO_WARNING %:p -o %:p:r && %:p:r < %:p:h/in <CR>", opt)
...
map("n", "<leader>fr", ":!cd %:p:h && cf test<CR> | less", opt)
